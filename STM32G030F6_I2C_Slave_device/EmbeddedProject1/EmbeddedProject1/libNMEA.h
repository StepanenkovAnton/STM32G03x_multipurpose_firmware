...
typedef enum {
	...
	NMEA_WRONG_DATA,
	/**< wrong data passed by user (of the library)*/
	NMEA_WRONG_CB_ID,
	/**< invalid CB ID*/
...
/*!
 * NMEA_data is a structure carrying all useful received data.
 */
typedef struct {
	/*!
	 * <pre>	
	...
	uint32_t	latitude_xxxxx_00000;
	uint32_t	latitude_00000_xxxxx;
	 ...
	 */
	//float		longitude;
	uint32_t	longitude_xxxxx_00000;
	uint32_t	longitude_00000_xxxxx;
...
typedef enum {
	SPEED_CHANGE_CB = 0,
	/**< Id of callback to change of speed event.*/
	SPEED_RISE_BARRIER_CB,
	/**< Id of callback to cross the rise speed barrier event.*/
...
/**
 * convertNMEAtoI2C is a function used for an 8-bit sequental I2C suitable representation of an NMEA strictured data.\n 
 * @nmea_input	strictured  decoded GPS data
 * @pI2C_output	an I2C byte sequence buffer
 */
